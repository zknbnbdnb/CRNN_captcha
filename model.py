...
def train(inputs, targets,encoder, decoder,encoder_optimizer,decoder_optimizer,
    ...
    # return loss.data[0],accuracy_clevel,accuracy # torch0.4 loss.data[0]
    return loss.item(),accuracy_clevel,accuracy # torch1.0 loss.item()

def evaluate(inputs,targets,encoder,decoder,criterion,use_cuda=False):
    encoder.train(False)
    decoder.train(False)
    loss = 0
    ...
    decoder.train(True)
    #
    # return loss.data[0],accuracy_clevel,accuracy,outputs # torch0.4 loss.data[0]
    return loss.item(),accuracy_clevel,accuracy,outputs # torch1.0 loss.item()

def test(inputs,encoder,decoder,use_cuda=False):
    encoder.train(False)
    decoder.train(False)
    loss = 0
    ...
    input = Variable(torch.zeros(batch_size).long())
    if use_cuda:
        input = input.cuda()
    ...
    #
    # return loss.data[0],accuracy_clevel,accuracy,outputs # torch0.4 loss.data[0]
    return outputs # torch1.0 loss.item()
